      ...
      <div className="container mx-auto px-0 md:px-0">
        ...
        >
          <Card className="bg-background border border-border rounded-xl w-full">
            <CardHeader>
              <CardTitle className="text-primary text-2xl md:text-3xl font-bold leading-relaxed max-w-xl mx-auto text-center md:text-left mb-4">{t_form.title}</CardTitle>
              <CardDescription className="text-muted-foreground text-lg md:text-xl leading-relaxed max-w-xl mx-auto text-center md:text-left mb-6">{t_form.description}</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField control={form.control} name="phone" render={({ field }) => (
                    <FormItem><FormLabel className="text-muted-foreground">{t_form.fields.phone.label}</FormLabel><FormControl><Input type="tel" placeholder={t_form.fields.phone.placeholder} {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="category" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-muted-foreground">{t_form.fields.category.label}</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="rounded-xl"><SelectValue placeholder={t_form.fields.category.placeholder} /></SelectTrigger>
                        </FormControl>
                        <SelectContent>
                  ...
                  <FormField control={form.control} name="tradeRegister" render={({ field: { onChange, value, ...rest } }) => (
                    <FormItem>
                      <FormLabel className="text-muted-foreground">{t_form.fields.tradeRegister.label}</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input type="file" className="rounded-xl pl-12 rtl:pr-12 rtl:pl-4 file:mr-4 rtl:file:ml-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => onChange(e.target.files)} {...rest} />
                  ...
                  <Button type="submit" className="w-full text-lg rounded-xl bg-primary text-primary-foreground font-bold btn-brand-hover flex items-center justify-center py-4 sm:py-0 transition-all duration-300" disabled={form.formState.isSubmitting}>
